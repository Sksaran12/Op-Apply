...
const mode = process.argv[2] || (process.env.DATABASE_URL && (process.env.DATABASE_URL.startsWith('postgres') || process.env.DATABASE_URL.startsWith('postgresql')) ? 'postgres' : 'sqlite');

console.log(`[Toggle DB] Selected database mode: ${mode.toUpperCase()} (source: ${process.argv[2] ? 'argument' : 'auto-detect'})`);
...
try {
  ...
  if (fs.existsSync(envPath)) {
    ...
    console.log('[Toggle DB] .env file not found, skipping environment file update.');
  }

  console.log(`[Toggle DB] Successfully toggled database configuration to: ${mode.toUpperCase()}`);
} catch (error) {
